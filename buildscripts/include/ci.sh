...
: "${MPV_ANDROID_ARCHES:=${ci_build_arches[*]}}"
...
if [[ "$1" == export || "$1" == install ]]; then
	...
	ci_cache_id="${ci_tarball}-abi-${ci_arch_tag}-sources-${native_source_id}.tgz"
fi
...
if [ "$1" = "export" ]; then
	...
	if [ -f "../../patches/mpv_video_shaders.patch" ]; then
		git -C deps/mpv apply --reverse --check ../../patches/mpv_video_shaders.patch 2>/dev/null || \
