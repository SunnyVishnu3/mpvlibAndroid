...
[ -f "$pc" ] || { echo "Missing libarchive.pc" >&2; exit 1; }
for lib in -lz -lbz2 -llzma -lzstd -liconv -lcharset; do
	if ! grep -q -- "^Libs:.*$lib" "$pc"; then
		${SED:-sed} -i.bak "/^Libs:/ s/$/ $lib/" "$pc"
		rm -f "$pc.bak"
	fi
done
PKG_CONFIG_LIBDIR="$prefix_dir/lib/pkgconfig" pkg-config --static --libs libarchive >/dev/null
